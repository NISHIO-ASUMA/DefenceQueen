//===================================================================
//
// インスタンシング用モーション管理クラス [ instancemotionmanager.cpp ]
// Author: Asuma Nishio
// 
//===================================================================
...
// 静的メンバ変数宣言
//*********************************************************
int CInstanceMotionManager::m_nNumAll = NULL; // 総数加算

//=========================================================
// コンストラクタ
//=========================================================
CInstanceMotionManager::CInstanceMotionManager() :m_FileData{}
{
...
// デストラクタ
//=========================================================
CInstanceMotionManager::~CInstanceMotionManager()
{
	UnLoad();
}
//=========================================================
// 全読み込み処理
//=========================================================
HRESULT CInstanceMotionManager::Load(void)
{
	// 値の初期化
	m_FileData.clear();
...
// 全破棄処理
//=========================================================
void CInstanceMotionManager::UnLoad(void)
{
	// 値のクリア
	m_FileData.clear();
}
	...
	for (int nCnt = 0; nCnt < static_cast<int>(m_FileData.size()); nCnt++)
	{
		...
		if (m_FileData[nCnt].FilePath == pFileName)
		{
			...
			for (int nModel = 0; nModel < static_cast<int>(m_FileData[nCnt].Modelpath.size()); nModel++)
			{
				...
				// インスタンシングモデルのポインタを生成
				CInstanceModel* pNewModel = CInstanceModel::Create(VECTOR3_NULL, VECTOR3_NULL, path.c_str(), isShadow);
...
// モデル数をチェックする
//=========================================================
int CInstanceMotionManager::SetModels(std::istringstream& iss)
{
	// 文字列設定
	std::string eq;
	int nModel = 0;
...
void CInstanceMotionManager::SetModelFile(std::istringstream& iss, std::vector<CInstanceModel*>& pModel, int nCnt, const bool isShadow)
{
	...
	// モデルの生成処理
	CInstanceModel* pNewModel = CInstanceModel::Create(VECTOR3_NULL, VECTOR3_NULL, filename.c_str(), isShadow);
...
// パーツごとの情報をセットする
//=========================================================
void CInstanceMotionManager::SetParts(std::ifstream& file, std::vector<CInstanceModel*>& pModel)
{
	// 読み込み文字列
	std::string line;
...
// パーツごとのモーション情報をセットする
//=========================================================
void CInstanceMotionManager::SetPartsMotion(std::ifstream& file, int nCntMotion)
{
	// 読み込み文字列
	std::string line;
...
// キー全体の情報をセットする
//=========================================================
void CInstanceMotionManager::SetKey(std::ifstream& file, int nCntMotion, int nCntKey)
{
	// 行読み込み
	std::string line;
